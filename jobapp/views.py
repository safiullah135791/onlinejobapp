...
def user_home(request):
    ...
    user = request.user
    user = StudentUser.objects.get(user=user)
    error = ""
    if request.method == 'POST':
       f =  request.POST['fname']
       l =  request.POST['lname']
...
def add_job(request):
    ...
    if request.method == 'POST':
        ...
        recruiter = Recruiter.objects.get(user=user)
        try:
            Job.objects.create(recruiter=recruiter,start_date=st,end_date=et,title=jt,salary=sal,image=l,description=des,experience=exp,location=loc,skills=skills,creationdate=date.today())
            error = 'no'
        except:
            error = 'yes'
    d = {'error':error}
...
def applied_candidateslist(request):
    if not request.user.is_authenticated:
        return redirect('recruiter_login')
    data = Apply.objects.all()
    d = {'data':data}
    return render(request,'applied_candidateslist.html',d)
